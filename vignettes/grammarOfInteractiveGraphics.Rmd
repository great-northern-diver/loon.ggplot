...
More meaningful date information can be added using some standard `R` functions:
```{r}
...
airquality$Weekday <- factor(weekdays(airquality$Date),
                             levels = c("Monday",
                                        ...
                                        "Sunday"))
```
The `Month` itself can also be tidied up, turning it into into a factor with levels having a three character month abbreviation (and ordered to preserve calendar order):
