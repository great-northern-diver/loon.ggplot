...
Size is another plot state that differs between `ggplot` and `loon`.  The point `size` in `loon` identifies the approximate **area** of the point symbol, so when transforming from `ggplot` to `loon`, the `size` parameter of the `ggplot` is transformed to an integer value in `loon` that tries to match the area of the point in `ggplot`.
...
The colour values are a little different.  
Visibly, there is a difference in the `"grey"` points seen in each plot.  The two plots have different values stored as their colour state:
```{r colours}
ggp_states$colour
lp["color"]
```

The `loon` plot colours are twelve hexadecimal digits, as in `tk` (e.g., see `tkcolors()`); the `ggplot` will have named `R` colours **or** six hexadecimal digits (two for each of the red, green, and blue components).  
...
This is because the **named** colours in `R` are slightly different from those in `Tcl` that `loon` is built on.  The `R` colours whose name **is the same** as that in `Tcl` **but whose colour is different** are **only** the following:
...
Some care needs to be taken when dealing with linking, especially when there is missing data.  See the vignette `Linking` and the `loon` vignette `Logical Queries` for more information.
...
```
The **shape** is now `pch = 16`, which in `R`, corresponds to a filled circle (without a boundary). 
...
lp_ggp_lp["selected"]
```
This is for two reasons.  First the selected points of `lp` changed to colour in `ggp_lp` and so the selected points from `lp` could not be transferred to `lp_ggp_lp`. The colours in `lp_ggp_lp` will not match those ofSecond, the new plot has not (yet) joined the same linking group as the original.
...
```{r loon to ggplot to loon, fig.width = 12, fig.height = 4, out.width = "100%", warning = FALSE}
# PROBLEM
library(grid)
lp["x"]
lp_ggp_lp["x"]
