...
# catch_bin_info is mainly used to catch the order of the bins
# since we users set ylim or xlim the bins would change
...
catch_bin_info.StatBin <- function(hist_values, hist_data, x.limits, y.limits,
  ...
  if (!is.null(ggObj$labels$fill)) {
    ...
    panel_i.fill_var <- fill_var[facet_id][in_limits]
    ...
    if (length(uni_fill) == length(levels)) {
      ...
      for(j in seq_len(length(levels))){
        fill[which(panel_i.fill_var %in% levels[j])] <- uni_fill[j]
      }
    }
  }
...
catch_bin_info.StatCount <- function(hist_values, hist_data, x.limits, y.limits,
  ...
  if(!is.null(ggObj$labels$fill) && ggObj$labels$fill != ggObj$labels$x) {
    ...
    panel_i.fill_var <- fill_var[facet_id]
    ...
    if(any(is.na(y))) {
      ...
      group_by_table <- data.frame(fill = panel_i.fill_var, hist_values = hist_values) %>%
      ...
      in_y.limits <- !((hist_values %in% drops) & (panel_i.fill_var %in% drop_fill))
    }

    in_limits <- in_y.limits & in_x.limits
    hist_values <- hist_values[in_limits]
    panel_i.fill_var <- panel_i.fill_var[in_limits]
    fill_levels <- rev(levels(as.factor(panel_i.fill_var)))
    ...
    in_limits <- in_y.limits & in_x.limits
    hist_values <- hist_values[in_limits]
    panel_i.fill_var <- hist_values
    fill_levels <- levels(as.factor(panel_i.fill_var))
  }
  ...
  if(length(uni_fill) == length(fill_levels)) {
    ...
    for(j in seq_len(length(fill_levels))){
      fill[which(panel_i.fill_var %in% fill_levels[j])] <- uni_fill[j]
    }
  } else {
    fill <- rep(uni_fill, length(hist_values))
  }
